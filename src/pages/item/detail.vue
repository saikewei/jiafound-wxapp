...
function goToDisputePage() {
  const claimId = currentClaimId.value;
  const itemID = item.value.itemID;

  console.log('[goToDisputePage] claimId:', claimId, 'itemID:', itemID);

  // 构建跳转参数
  let url = '/pages/dispute/applyDispute?';
  if (claimId) {
    url += `claimId=${claimId}`;
  } else {
    uni.showToast({ title: "缺少必要信息，无法发起维权", icon: "none" });
    return;
  }
  uni.navigateTo({
    url,
    fail: (err) => {
      console.error("跳转纠纷页失败:", err);
...
async function fetchDetail() {
  ...
  try {
    ...
    if (statusRes.status === 'fulfilled') {
      ...
      currentClaimId.value = statusData.claimId || "";
